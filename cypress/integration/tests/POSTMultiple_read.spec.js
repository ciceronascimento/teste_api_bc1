import * as MultiReadContract from '../requests/POSTMultiple_read.request';
// const data = require('../payloads/add-data.json')

describe('GET Multiple Read Contract', () => {
    it('Listar documento criado em um contrato', () => {
        MultiReadContract.retornarDocumentoDoContratato().should((response) => {
    ...
    //     MultiReadContract.retornarDocumentoDoContratatoComTokenInválido().should((response) => {
    ...
    //     MultiReadContract.retornarDocumentoDoContratatoComParametrosVazios().should((response) => {
    //         expect(response.status).to.eq(401);
    ...
    //     MultiReadContract.retornarDocumentoDoContratatoSemParametros().should((response) => {
    //         expect(response.status).to.eq(401);
    //         expect(response.body.result.code).to.eq("CAMPO_VAZIO");
    //         expect(response.body.result.message).to.eq("Preencha todos os campos!");
    //     })
